...
from utils.losses import Conditional_Contrastive_loss, Conditional_Contrastive_loss_plus, Proxy_NCA_loss, NT_Xent_loss, SupConLoss
...
class make_worker(object):
    def __init__(self, cfgs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model, inception_model, Gen_copy,
        ...
        if self.conditional_strategy == 'ContraGAN':
            ...
            self.contra_plus_criterion = Conditional_Contrastive_loss_plus(self.rank, self.batch_size)
                ...
                for acml_index in range(self.accumulation_steps):
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            cls_proxies_real, cls_embed_real, dis_out_real = self.dis_model(real_images, real_labels, real=True)
                            cls_proxies_fake, cls_embed_fake, dis_out_fake = self.dis_model(fake_images, fake_labels, real=False)
                        else:
                            raise NotImplementedError
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            dis_acml_loss += self.contrastive_lambda*\
                                (self.contra_plus_criterion(cls_embed_real, cls_proxies_real, real_cls_mask, real_labels, t) +\
                                 self.contra_plus_criterion(cls_embed_fake, cls_proxies_fake, fake_cls_mask, fake_labels, t))
                        else:
                            pass
                        ...
                        if self.cr:
                            ...
                            elif self.conditional_strategy in ["NT_Xent_GAN", "Proxy_NCA_GAN", "ContraGAN", "ContraGAN_plus"]:
                                _, cls_embed_real_aug, dis_out_real_aug = self.dis_model(real_images_aug, real_labels)
                            ...
                            if self.conditional_strategy in ["ACGAN", "NT_Xent_GAN", "Proxy_NCA_GAN", "ContraGAN", "ContraGAN_plus"]:
                        ...
                        if self.zcr:
                            ...
                            if self.conditional_strategy == "ACGAN":
                                ...
                                _, cls_embed_fake_zaug, dis_out_fake_zaug = self.dis_model(fake_images_zaug, fake_labels)
                ...
                for acml_step in range(self.accumulation_steps):
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            cls_proxies_fake, cls_embed_fake, dis_out_fake = self.dis_model(fake_images, fake_labels, real=True)
                        else:
                            raise NotImplementedError
                        ...
                        if self.conditional_strategy == "ACGAN":
                            ...
                            fake_cls_mask = make_mask(fake_labels, self.num_classes, self.rank)
                            gen_acml_loss += self.contrastive_lambda*self.contra_plus_criterion(cls_embed_fake, cls_proxies_fake, fake_cls_mask, fake_labels, t)
                        else:
                            pass
